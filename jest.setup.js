...
// ReadableStream polyfill (Node.js 환경에서 필요)
if (!global.ReadableStream) {
  const { ReadableStream } = require("stream/web");
  global.ReadableStream = ReadableStream;
}

// TransformStream polyfill (MSW/interceptors 요구사항)
if (!global.TransformStream) {
  const { TransformStream } = require("stream/web");
  global.TransformStream = TransformStream;
}

// MessagePort/MessageChannel polyfill (undici 요구사항)
...
// BroadcastChannel polyfill (MSW v2 WebSocket 기능 요구사항)
...
// 환경변수 설정 (테스트 환경 - 기본값만 제공, 개별 테스트에서 덮어쓰기 가능)
