...
export const POST_QUERY_KEYS = {
  ...
  list: (params?: any) => [...POST_QUERY_KEYS.lists(), params] as const,
 ...
 * 
 * @param params - 페이지네이션, 정렬, 태그 필터링 매개변수
 */
export function usePosts(params?: {
  ...
  return useQuery({
    ...
    // postService.getPosts 호출 시 매개변수 전달
    ...
    staleTime: 2 * 60 * 1000, // 2분 - 게시글 목록은 자주 변할 수 있음
    retry: 2,
...
export function useInfinitePosts(params?: {
  ...
  return useInfiniteQuery({
    queryKey: POST_QUERY_KEYS.list({ ...params, infinite: true }),
...
export function useCreatePost() {
  ...
  return useMutation({
    ...
    onSuccess: (newPost: CreatePostResponse) => {
...
/**
 * 게시글 목록을 수동으로 새로고침하는 함수를 반환하는 훅
 * 
 * 특정 상황에서 수동으로 게시글 목록을 갱신하고 싶을 때 사용합니다.
 */
export function useRefreshPosts() {
  const queryClient = useQueryClient();

  const refreshPosts = () => {
    // 모든 게시글 목록 쿼리를 무효화하여 재요청 유도
    queryClient.invalidateQueries({
      queryKey: POST_QUERY_KEYS.lists()
    });
  };
